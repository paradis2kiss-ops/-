...
import { LibraryBig, MapPin, Moon, Sun, RotateCcw, AlertTriangle, PenLine, GripHorizontal } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-1000 font-body relative overflow-x-hidden
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8 md:p-8 flex flex-col justify-center min-h-screen">
        
        <div className="flex justify-center w-full">
          {/* Main Container: Parchment Roll on Mobile, Open Book on Desktop */}
          <div className={`w-[94%] md:w-full max-w-4xl lg:max-w-7xl transition-all duration-500 animate-fade-in-up
              ${isDarkMode 
                ? 'bg-night-card backdrop-blur-md border border-white/20' 
                : 'bg-day-card'
              }
              /* Mobile: Parchment Styles */
              rounded-t-[3rem] rounded-b-[3rem] shadow-parchment
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row">
              ...
              <div className={`w-full lg:w-1/2 flex flex-col lg:pr-12 transition-all duration-300 relative 
                ...
                {/* 1. Header Section */}
                <div className="flex flex-col items-center justify-center mb-8 lg:mb-10 text-center pt-2">
                  <div className={`p-3 rounded-full mb-4 shadow-sm ${isDarkMode ? 'bg-white/10 backdrop-blur-sm' : 'bg-white/60'}`}>
                      <LibraryBig className={`w-10 h-10 ${isDarkMode ? 'text-white' : 'text-day-accent'}`} />
                  </div>
                  <h1 className={`text-4xl md:text-5xl font-body font-bold tracking-tight mb-3 ${isDarkMode ? 'text-white drop-shadow-md' : 'text-day-text'}`}>
                    종이약국
                  </h1>
                  <p className={`text-lg font-surround opacity-90 ${isDarkMode ? 'text-night-sub' : 'text-day-sub'}`}>
                      당신의 감정을 위한 책 처방전
                  </p>
                </div>

                {/* Mobile Divider */}
                <div className={`lg:hidden w-full h-px mb-8 mx-auto max-w-[80%] opacity-30 border-t border-dashed ${isDarkMode ? 'border-white' : 'border-day-text'}`}></div>

                {/* 2. Mood Section */}
                <div className="mb-8 lg:mb-10">
                  <div className="flex justify-between items-center mb-4">
                     <label className={`block text-xl font-surround font-bold ${isDarkMode ? 'text-white' : 'text-day-text'}`}>지금 기분은 어때요? *</label>
                  ...
                  <div className="grid grid-cols-3 gap-3 lg:gap-4">
                    {moodOptions.map(mood => (
                      <button 
                          ...
                          className={`group p-2 sm:p-3 flex flex-col items-center justify-center rounded-2xl border transition-all duration-300 hover:scale-105 hover:shadow-md min-h-[110px] lg:min-h-[130px]
                ...
                <div className="mb-8 lg:mb-0">
                  <label className={`block text-xl font-surround font-bold mb-3 flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-day-text'}`}>
                  ...
                  <input 
                      ...
                      className={`w-full p-4 rounded-xl border transition-all duration-300 focus:outline-none font-body text-lg
                          ${isDarkMode 
                              ? 'bg-night-btn border-transparent text-white placeholder-white/30 focus:border-white/50' 
              ...
              <div className="w-full lg:w-1/2 lg:pl-12 flex flex-col">
                ...
                {/* 4. Situation Section */}
                <div className="mb-8 lg:mb-10 lg:mt-0">
                  <label className={`block text-xl font-surround font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-day-text'}`}>지금 상황을 편하게 말해줘요</label>
                  <textarea 
                      value={userInput.situation} 
                      onChange={(e) => handleInputChange('situation', e.target.value)} 
                      placeholder="예: 요즘 취준 때문에 너무 힘들어요... 위로가 필요해요 😢" 
                      className={`w-full p-5 rounded-xl border transition-all duration-300 focus:outline-none resize-none font-body text-lg min-h-[160px] leading-relaxed
                          ${isDarkMode 
                              ? 'bg-night-btn border-transparent text-white placeholder-white/30 focus:border-white/50' 
                              : 'bg-day-bg border-transparent text-day-text placeholder-day-sub/50 focus:border-day-accent'
                          }`} 
                ...
                {/* 5. Genre Section */}
                <div className="mb-10">
                  <label className={`block text-xl font-surround font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-day-text'}`}>선호하는 장르는? (선택)</label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                    {genreOptions.map(genre => (
                      <button 
                          ...
                          className={`px-3 py-3 rounded-xl text-sm font-surround font-bold transition-all duration-300 hover:scale-105 shadow-sm whitespace-normal break-keep flex flex-col items-center justify-center text-center h-[90px] border
                ...
                <div className="mt-auto">
                    <div className="mb-6">
                        <div className="flex items-center justify-between mb-2">
                            <label className={`block text-lg font-surround font-bold opacity-90 ${isDarkMode ? 'text-white' : 'text-day-text'}`}>도서관 위치 설정</label>
                        </div>
                        <button 
                            type="button" 
